...
!!! ms-abstract ""
    ...
    **MeterSphere 版本号命名规则为：**  v *大版本号*.*功能版本号*.*修复版本号*，目前已经发布了三个 LTS 版本：
...
!!! ms-abstract "功能优化 :sunflower:"
    ...
    - perf(测试跟踪)：步骤描述用例交互优化
    - perf(测试跟踪)：用例评审的高级搜索功能增加用例表单字段
    - perf(接口测试)：接口参数大数据量的性能优化


!!! ms-abstract "问题修复 :palm_tree:"

    - fix(测试跟踪)：修复测试用例的模块名称修改后，模块路径显示错误的问题 #35531
    - fix(测试跟踪)：修复导入测试用例时，连续点击两次会重复导入测试用例的问题 #35465
    - fix(测试跟踪)：修复在【项目设置】→【模板管理】→【系统默认字段】中新增选项值后，在【测试跟踪】→【缺陷管理】中无法筛选出原有选项值的问题 #35407
    - fix(测试跟踪)：修复脑图用例视图切换列表用例视图失败的问题 #34921
    - fix(测试跟踪)：修复导出Excel用例时，多步骤用例字段自动合并单元格的问题 #34866
    - fix(测试计划)：修复在修改性能测试用例时，修改配置界面中的“高级配置”选项卡无法加载CSVDataSet设置的问题 #35454
    - fix(测试计划)：修复接口场景自定义ID显示不正确的问题 #35079
    - fix(测试计划)：修复消息通知的Stage无法获取自定义测试阶段的值的问题 #34859
    - fix(接口测试)：修复接口定义SQL导出再导入时，原接口Case被迁移到新导入的接口里的问题 #35464
    - fix(接口测试)：修复接口场景总步骤数量较多时，在列表页复制子场景，页面卡住，不会自动刷新的问题 #35403
    - fix(接口测试)：修复接口自动化场景中，被引用的SQL协议接口Case无法添加前后置操作或者断言操作的问题 #35200
    - fix(接口测试)：修复导入JMeter 5.5版本的循环控制器文件时，提示解析失败的问题 #35191
    - fix(UI测试)：修复删除元素库中被引用的一个元素时，定位类型发生改变的问题 #35467
    - fix(UI测试)：修复在高级设置的后置设置中对元素添加断言后，页面上的元素名称和高级设置列表名称无法对应的问题 #35439
    - fix(UI测试)：修复“编辑”按钮显示不完整的问题 #35415
    - fix(UI测试)：修复在UI测试的自动化编写过程中切换场景时，显示的场景内容混乱的问题 #35392
    - fix(性能测试)：修复OAuth2登录后，在性能测试页面上传2KB的JMX文件时，提示“上传文件大小不能超过 MB!”信息的问题 #35391
    - fix(性能测试)：修复性能测试报告中“测试概览”页和“测试详情”页中统计的用户并发数与图表数据不匹配的问题 #34844
    - fix(项目设置)：修复连续添加两个相同变量，删掉第一个相同变量，点击“保存”按钮后，再次编辑环境时，没有新的空白变量入口的问题 #35398
...
!!! ms-abstract "问题修复 :palm_tree:"
    ...
    - fix(工作台): 修复我的待办功能用例数据错误问题
    - fix(接口测试): 接口导入问题
    - fix(接口测试): 场景步骤重复 key 处理
    - fix(接口测试): 修复接口导出再导入问题
    - fix(接口测试): 场景保存，接口响应超时优化
    - fix(测试跟踪): 功能用例导出为excel-执行结果字段为「未执行」时-在excel内查看为空
    - fix(测试跟踪): 执行中的报告禁止重跑
    - fix(测试跟踪): 修复测试计划报告概率性被提前结束
    - fix: 附件转存问题修复
...
!!! ms-abstract "问题修复 :palm_tree:"
    ...
    - fix(接口测试): 修改接口路径匹配算法
    - fix(接口测试): 修复场景复制自定义请求时会造成接口覆盖率不准确的问题
    - fix(测试跟踪): 功能用例脑图保存遮罩
    - fix(测试跟踪): 功能用例脑图查看一直loading
    - fix(测试跟踪): 功能用例高级搜索结果有误
    - fix(测试跟踪): 功能用例创建通知，跳转详情报错
    - fix(测试跟踪): 关联需求检索后，文字太长会出现重叠
    - fix(测试跟踪): 用例评审，没有写评论直接通过，后台日志报错
    - fix(测试跟踪): 首页用例数统计点击跳转，模块树数量统计有误
    - fix(测试跟踪): 工作台缺陷列表，成员类型的自定义字段，显示的是ID不是名字
    - fix(性能测试): 调整性能测试文件上传窗口组件的叠放等级
