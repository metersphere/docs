...
- refactor(测试计划): 优化网络波动或资源池重启造成测试计划执行时间过长问题
- refactor(接口测试): 资源池反显优化
- refactor(接口测试): 场景步骤过多时添加提示
- refactor(接口测试): 去除本地执行，统一放到资源池执行
- refactor: 处理分享报告查询资源池的返回信息

!!! info "问题修复"
- fix(接口测试): 修复禁用local资源池，执行case还是显示local资源池的缺陷
- fix(接口测试): 修复前端精度问题
...
- fix(测试跟踪): 修复选择资源池执行后仍选择LOCAL资源池执行的缺陷
- fix(测试跟踪): 修改测试计划报告的完成率为执行完成率
- fix(测试跟踪): 修复测试计划分享报告无法加载场景步骤内容的问题
- fix(测试跟踪): 评审状态为已完成，再次关联用例后取消关联，评审状态变成了进行中
- fix(项目设置): 更改性能测试脚本预警的方式
...
- fix(测试计划): 修复三方流水线并发调用同一个测试计划执行偶发数据库LOCK WAIT问题
- fix(项目设置): 修复首次登入后，偶现后台报错问题
- fix(系统设置): 修复更新当前站点未同步TCP-MOCK环境问题
...
!!! info "功能优化"
-  refactor(系统设置): 优化系统参数设置保存时更改mock环境的方法
-  refactor(接口测试): 重复导入相同接口处理逻辑优化
-  style(接口测试): 接口用例编辑页面最大化按钮位置优化
-  refactor(性能测试): 增加压力参数校验

!!! info "问题修复"
-  fix(测试跟踪): 修复模块添加八层页面报500的缺陷
-  fix(测试跟踪): 修复复制测试计划多出来功能用例的缺陷
-  fix(测试跟踪): 修复脑图拖拽用例到已删除的用例上边，保存会报错问题
-  fix(测试跟踪): 修复测试计划分享报告中接口用例执行报告总是显示最新报告的问题
-  fix(测试跟踪): 修复评论无法展示多张图片问题
-  fix(测试跟踪): 修复用例评审关联用例的评论编辑问题
-  fix(接口测试): 修复引用步骤中添加等待控制器执行不生效问题
-  fix(接口测试): 修复系统自动生成的定时任务保存或重置报错的缺陷
-  fix(接口测试): 修复文件导入后再次被修改保存报错问题
-  fix(接口测试): 修复case按标签全选后搜索，批量执行报错的缺陷
-  fix(接口测试): 修复全局断言中响应时间配置不生效的缺陷
-  fix(接口测试): 修复场景中包含特殊字符$生成报告失败的缺陷
-  fix(接口测试): 修复接口导出swagger3.0部分响应体丢失问题
-  fix(接口测试): 修复环境批量添加参数解析换行没有兼容windows的缺陷
-  fix(项目设置): 修复全局断言脚本内容未及时刷新的缺陷
-  fix(项目设置): 修复环境保存http保存失败的缺陷
-  fix(项目设置): 修复定时清理接口报告条目太多不能正常执行的问题
...
!!! info "功能优化"
- refactor(性能测试): 使用K8S资源池可以配置 Job 模版
- refactor(性能测试): 报告显示时间为 x 时 x 分 x 秒
- refactor(接口测试): 增加授权校验
- refactor(接口测试): 优化日志抛出异常信息
- style(UI自动化): 元素库定义多个层级并且名字很长，在场景中选择元素对象，下拉框数据没有显示完整
- style(接口测试): 优化任务中心场景报告显示样式
- style(测试跟踪): 开启定时任务提示信息样式重叠修改
- feat(接口测试): mock 期望的延迟响应时间 tab 增加时间单位
- feat(接口测试): 接口测试支持禁用本地执行

!!! info "问题修复"
- fix(UI 自动化): UI 场景支持定时任务
- fix(UI 自动化): UI 自动化定时任务配置页面显示优化
- fix(UI 自动化): 删除 UI 定时任务任务通知运行配置项目
- fix(UI 自动化): 测试报告-触发方式- 不能根据定时任务进行筛选
- fix(接口测试): 修复 mock 函数拼接值出现 undefined 问题
- fix(接口测试): 修复 jsonschema 高级设置属性丢失的缺陷
- fix(接口测试): 修复 node 执行完成后发通知获取用户异常
- fix(接口测试): 修复 swagger 文件导入 query 参数默认值导入失败问题
- fix(接口测试): 修复断言 root 层级数据添加期望值保存失效的缺陷
- fix(接口测试): 修复引用的 case 包含文件时可以修改的缺陷
- fix(接口测试): 修复接口导入 swagger3 文件报错问题
- fix(接口测试): 修复复制的场景中引用的 case 有时不能操作的缺陷
- fix(接口测试): 修复断言添加子字段无效的缺陷
- fix(接口测试): 修复引用的场景可以上传文件的缺陷
- fix(接口测试): 修复用例中高级设置页面被遮挡问题
- fix(接口测试): 修复状态不统一导致报告出现空白问题
- fix(接口测试): 禁用接口执行调用自身引发死循环问题
- fix(接口测试): 修复测试计划中场景执行报告内容未加载问题
- fix(接口测试): 修复同名数据源执行过程产生大量错误日志问题
- fix(接口测试): 修复查看没有入库的接口报告时前端报错的缺陷
- fix(接口测试): 修复无项目权限的操作人执行提示没有环境的缺陷
- fix(接口测试): 修复选中一条接口用例使用批量编辑用例等级，结果全部用例被修改的缺陷
- fix(性能测试): 修复导出报告图错乱的问题
- fix(测试跟踪): 高级搜索创建人列表问题
- fix(测试跟踪): 查询禅道需求时包含子需求
- fix(测试跟踪): 功能用例的关联需求显示有误
- fix(测试跟踪): 测试计划报告执行率统计有误
- fix(测试跟踪): 禅道同步缺陷后图片无法展示
- fix(测试跟踪): 依赖关系图显示了下层富文本框
- fix(测试跟踪): 测试计划执行结束时入库增加判断
- fix(测试跟踪): 修复测试计划批量执行报告查看异常问题
- fix(测试跟踪): 禅道缺陷上传图片链接后同步，图片不显示
- fix(测试跟踪): 优化测试计划保存最后总计算结果时的判断逻辑
- fix(测试跟踪): 测试报告里的通过率再存放数据库的单独字段中
- fix(测试跟踪): 修复测试计划报告内，未执行的场景图标没有统计的问题
- fix(测试计划): 执行功能用例时实际结果与步骤执行结果无关联权限无法编辑
- fix(接口定义): 修复自定义请求变量缺失的缺陷
...
- refactor(测试跟踪): 测试计划用例标题显示优化
- refactor(消息通知): 统一发送邮件时的标题为中文
- refactor(接口测试): 优化缓存取值失败后再从数据库取值，不影响执行

!!! info "问题修复"
- fix(UI 自动化): 遗留 UI 国际化问题
- fix(UI 自动化): 修复 UI 报告详情展示
- fix(UI 自动化): 修复 UI 报告分享链接有效期设置没生效
- fix(测试跟踪): Jira 项目集字段同步有误
- fix(测试跟踪): Jira 没勾选第三方平台模板，提交缺陷图片不显示
- fix(测试跟踪): 优化测试计划报告查看速度
- fix(测试跟踪): 自定义字段默认值设置无效
- fix(测试跟踪): 测试计划自动更新功能用例状态失效
- fix(测试跟踪): 测试计划自动更新功能用例状态失效
- fix(测试跟踪): 测试计划功能用例关系图显示了富文本框
- fix(接口测试): get 方式的 mock 接口，不设置任何匹配条件时，响应体为空
- fix(接口测试): 解决 TCP 协议导入其他设置丢失数据问题
- fix(接口测试): 修复 json 和 json-schema 转换时多层数组结构出现的问题
- fix(接口测试): 修复接口 json-schema 参数高级设置关闭未销毁问题
- fix(接口测试): 修复场景导入 har 文件丢失 formdata 问题
- fix(接口测试): 解决接口导入 json-schema 时没有 properties 属性数据会错乱问题
- fix(接口测试): 解决接口测试 swagger2 导入丢失描述问题
- fix(接口测试): 解决接口测试 swagger3 导入丢失非引用属性的值的问题
- fix(接口测试): 解决多版本创建问题
- fix(接口测试): 修复场景导入引用用例 id 与生成的用例 id 不一致问题
- fix(接口测试): 项目只读用户进入接口定义页面自动跳转到系统设置
- fix(接口定义): 修复 csv 无法预览的缺陷
- fix(接口定义): 修复 har 文件导入执行提示没环境的缺陷
- fix(接口定义): 修复 json-schema 没有国际化的缺陷
- fix(接口定义): 修复 json 格式错误仍可以转 json-schema 的缺陷
- fix(接口定义): 修复文档筛选不生效的问题
- fix(接口定义): 修复自定义请求变量缺失的缺陷
- fix(接口定义): 修复导入的接口定义无法添加文件的缺陷
- fix(接口定义): 接口定义列表用例通过率和描述去掉排序
- fix(接口定义): 接口定义列表用例通过率和描述去掉排序
- fix(系统设置): 修复复制环境，切换项目 http 请求头丢失的缺陷
- fix(性能测试): 测试命长较长时生成报告报 SQL 错误
...
-  fix(测试跟踪): 依赖关系列表布局问题 (#18276)
-  fix(测试跟踪): 回收站功能用例筛选问题 (#18034)
-  fix(测试跟踪): 修复关联场景执行状态不一致的缺陷 (#18065)
-  fix(测试跟踪): 测试计划及用例评审依赖关系的显示问题 (#18277)
-  fix(测试跟踪): 修复测试计划切换环境，接口用例切换数据源失败的缺陷 (#17924)
-  fix(接口定义): 修复自定义tcp没名字时无法执行的缺陷 (#18103)
-  fix(接口定义): 修复测试计划case列表页执行结果状态与接口定义case不一致的缺陷 (#18117)
-  fix(接口定义): 修复接口定义添加body文件切换到test文件丢失的缺陷 (#18174)
-  fix(接口定义): 修复分享场景报告title和页面控制台报错问题 (#18121)
-  fix(接口定义): 修复接口定义创建新mock时右上角保存与缩放连接在一起的样式缺陷 (#18179)
...
-  feat(接口测试): 接口导入，如果导入的接口没有用例，且这个接口为新建的，那需要为这个接口新建一-  个用例
-  refactor(UI自动化): 代码风格优化
...
-  refactor(接口导入): 接口导入覆盖模式规则优化

!!! info "问题修复"
-  fix(idea插件): 导入接口空指针报错
-  fix(接口测试): postman文件导入报错问题
-  fix(接口测试): 修复导入ms格式的带用例的接口定义，用例的创建人不是导入用户
-  fix(接口测试): 修复场景变量添加csv未上传文件不提示的缺陷 (#16663)
-  fix(接口测试): 修复场景变量列表错位的缺陷
...
-  fix(系统设置): 无项目权限时在项目管理菜单点击成员会被重定向
...
!!! info "功能优化"
-  refactor(接口测试): form表单校验信息过长时显示优化
-  refactor(接口测试): 兼容第三方jar包版本不同冲突的缺陷
-  refactor(接口测试): 优化结果报告统计减少和数据库交互
-  refactor(接口测试): 解决导入文件有缺失时系统报错问题
-  refactor(接口测试): 改成同步发送执行指令，调整连接参数
-  refactor(接口测试): 接口存在多个版本时不能通过创建接口创建
-  refactor(接口测试): 增加接口导入响应体的对比判断以及过滤多余代码
...
-  fix(接口测试): 修复导入JMeter脚本文件
-  fix(接口测试): 接口导入esb导入优化
...
-  fix(接口定义): 修复sql加载用例，sql脚本没有同步的缺陷 (#16482)
-  fix(接口定义): 修复除http协议TEST页面加载用例显示成了新建用例页面的缺陷
-  fix(接口定义): 修复场景关联case搜索失效的缺陷
...
-  fix(接口定义): 修复接口报告高级搜索触发方式只有手动触发的缺陷 (#16021)
-  fix(项目设置): 修复环境配置中http配置模块条件控制台报错的缺陷 (#15991)
