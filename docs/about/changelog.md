...
-  refactor(场景自动化): 场景报告跳转到场景列表打开新的 tab 页
-  refactor: 前端输入框 Mock 补全 JMeter 内置函数的提示

!!! info "问题修复"
-  fix(测试跟踪): Jira 同步经办人及项目集字段有误 (#18280)
-  fix(测试跟踪): 自定义 ID 开启后用例列表 ID 显示问题 (#18012)
...
-  fix(接口定义): 修复自定义 tcp 没名字时无法执行的缺陷 (#18103)
-  fix(接口定义): 修复测试计划 case 列表页执行结果状态与接口定义 case 不一致的缺陷 (#18117)
-  fix(接口定义): 修复接口定义添加 body 文件切换到 test 文件丢失的缺陷 (#18174)
-  fix(接口定义): 修复分享场景报告 title 和页面控制台报错问题 (#18121)
-  fix(接口定义): 修复接口定义创建新 mock 时右上角保存与缩放连接在一起的样式缺陷 (#18179)
