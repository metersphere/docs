...
## 1 用户
!!! ms-abstract ""
    点击左侧【系统设置-系统-用户】进入用户管理界面。默认显示当前系统中的全部用户列表，可以对用户进行【创建用户】【邮箱邀请】【导入用户】【修改用户组】【查询】【编辑】【重置密码】【禁用】【删除】等操作。MeterSphere 部署成功后会自动创建一个系统管理员账户，用户名为 admin，默认密码为 metersphere。如将平台用于生产环境，请及时更改密码。
