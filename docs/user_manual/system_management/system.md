...
!!! ms-abstract ""
    系统主要针对系统级别的管理配置功能。目前，MeterSphere 内置三级租户体系。平台默认用户组划分为：<br>
     - 【系统级用户组】：系统管理员、系统成员<br>
     - 【组织用户组】：组织管理员、组织成员。<br>
     - 【项目用户组】：项目管理员、项目成员。<br>
    系统的用户权限【用户组】功能页面中自定义，系统预置管理员角色权限不可编辑修改。

## 1 用户
!!! ms-abstract ""
    点击左侧【系统设置】下拉菜单中的【系统】进入【用户】界面。默认显示当前系统中的全部用户列表，可以对用户进行【新增】、【邮箱邀请】、【导入用户】、【修改用户组】、【查询】、【编辑】、【重置密码、【禁用】、【删除】等操作。MeterSphere 部署成功后会自动创建一个系统管理员账户，用户名为 admin，默认密码为 metersphere。如将平台用于生产环境，请及时更改密码。
![!用户管理](../../img/system_management/系统用户.png){ width="900px" }

!!! ms-abstract ""

    - **创建用户**<br>
    点击【创建用户】按钮创建用户，在弹出页面中编辑用户信息，可以批量【添加用户】并设置【用户组】权限，保存后添加为平台用户。注意：初始密码为邮箱地址。
