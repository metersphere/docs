...
!!! ms-abstract "操作说明"
    ...
    -  【编辑】：修改名称、组织管理员、描述。
    -  【添加成员】: 给当前组织添加成员用户和用户组。</br>
    -  【结束】：结束组织不展示在组织切换列表。</br>
    -  【查询】：列表右上方书输入框使用 ID 、名称查询组织。</br>

!!! ms-abstract "添加成员"
    点击【添加成员】按钮，勾选成员后，选择用户组，点击添加即可。
![!工作空间管理](../../img/system_management/组织添加成员.png){ width="900px" }
...
!!! ms-abstract "添加成员"
    点击【添加成员】按钮，勾选成员后，选择用户组，点击添加即可。
![!工作空间管理](../../img/system_management/项目添加成员.png){ width="900px" }
...
!!! ms-abstract "操作说明"
    ...
    - 【资源池】：用于接口测试执行任务，可选择全部资源池和指定资源池。资源池配置在[系统-资源池](./system.md#5)配置。</br>
    - 【状态】：默认启用，有启用，关闭两种状态。</br>


## 4 系统参数
!!! ms-abstract ""
    点击左侧【系统设置-系统-系统参数】进入参数设置界面，用于平台【基本设置】、【模型设置】、【数据清理】等参数的设置。</br>
...
### 4.3 数据清理
!!! ms-abstract ""
    - 【日志清理】：可以设置系统日志保留时长，定时每天 0 点定时清理。</br>
