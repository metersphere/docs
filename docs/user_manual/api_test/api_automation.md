...
### 1.1 步骤
!!! ms-abstract ""
    如下图，【添加步骤】可以添加请求、逻辑控制器和其他操作，详情参考：[场景步骤](./api_automation.md#2)
...
### 1.2 参数 
!!! ms-abstract ""
    如下图，【参数】可设置场景中请求使用的常量、列表参数进行传参
![!场景步骤](../../img/api_test/api_automation/场景参数设置.png){ width="900px" }

!!! ms-abstract "注意"
    尽量避免使用同名变量，使用同名变量时，场景级 CSV 优先级最高 <br>
    场景参数优先级：临时参数 > 场景参数 > 环境参数 > 全局参数 <br>
    **临时参数**: vars.put() 方式存储参数 和 后置操作提取的参数，一般采用就近原则使用参数。

### 1.3 前/后置
!!! ms-abstract ""
    如下图，场景前后置可以进行脚本操作和设置等待时间。详情参考：[前后置操作](../extend_features#1)
![!场景前后置](../../img/api_test/api_automation/场景前后置.png){ width="900px" }

### 1.4 断言
!!! ms-abstract ""
    如下图，场景有多种断言方式，如状态码、响应头、响应体、响应时间、脚本等。详情参考：[断言](../extend_features#3)
![!场景断言](../../img/api_test/api_automation/场景断言.png){ width="900px" }

### 1.5 设置
!!! ms-abstract ""
    如下图，【设置】处可进行 Cookie 配置、等待时间配置、步骤执行规则配置
...
!!! ms-abstract ""
    如下图，导入引用场景后，可配置场景引用模式、参数取值规则等内容
...
### 2.2 自定义请求
!!! ms-abstract ""
    如下图，【自定义请求】的创建使用与快捷调试请求类似。详情参考：[快捷调试请求](../debug#1)
...
### 2.3 循环控制器
!!! ms-abstract ""                                               
    如下图，【循环控制器】包括次数循环、while 循环、forEach 循环，详情参考：[循环控制器使用教程](https://kb.fit2cloud.com/?p=247)
![!循环控制器](../../img/api_test/api_automation/循环控制器.png){ width="900px" }

!!! ms-abstract ""
    【次数循环】，如下图，将请求拖拽到控制器里，设置循环次数、间隔时间等
![!次数循环](../../img/api_test/api_automation/次数循环.png){ width="900px" }

!!! ms-abstract ""
    【while 循环】，如下图，将请求拖拽到控制器里，设置表达式类型、循环条件、超时时间
...
!!! ms-abstract ""
    【forEach 循环】，如下图，将请求拖拽到控制器里，设置 forEach 遍历变量、时间间隔
...
### 2.4 条件控制器
!!! ms-abstract ""
    如下图，【条件控制器】设置条件后，只有满足设定的条件才会进入控制器里，执行里面的请求
![!条件控制器](../../img/api_test/api_automation/条件控制器.png){ width="900px" }

### 2.5 仅一次控制器
!!! ms-abstract ""
    如下图，请求需要拖拽进【仅一次控制器】里，确保所包含的测试步骤仅执行一次
![!条件控制器](../../img/api_test/api_automation/条件控制器.png){ width="900px" }
...
### 3.1 服务端执行
!!! ms-abstract ""
    如下图，点击【服务端执行】即可在资源池上执行场景，默认服务端为默认资源池
![!服务端执行](../../img/api_test/api_automation/服务端执行.png){ width="900px" }

!!! ms-abstract "注意"
    - **新增资源池**：除了默认资源池，可新增其他资源池，详情参考：[新增资源池](../system_management/system#5)
    - **切换资源池**：可以切换其他资源池进行使用，详情参考：[切换资源池](../project_management/project_permissions#23)

### 3.2 本地执行
!!! ms-abstract ""
    如下图，点击【本地执行】即可在本地服务执行场景。
...
## 4 场景功能
!!! ms-abstract ""
    如下图，单步骤可以进行【启用/禁用】、【执行】、【复制】、【删除】、【添加子步骤】、【在之前插入步骤】、【在之后插入步骤】操作
