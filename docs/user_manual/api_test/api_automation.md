...
!!! ms-abstract "" 
	...
	-  【资源池运行】：支持选择不同节点执行场景，可以选择普通资源池和 K8S 资源池，K8S 资源池是企业版功能。<br>
...
![!依赖关系图](../../img/api/接口自动化模块/依赖关系图.png){ width="900px" }

### 2.14 批量添加测试计划
!!! ms-abstract "" 
    在场景列表中选中多个场景后点击批量操作按钮中的【批量添加测试计划】，支持将多个场景添加到测试计划。

![!快速创建场景](../../img/api/接口自动化模块/批量添加测试计划001.png){ width="900px" }

!!! ms-abstract "" 
    勾选要添加的测试计划，点击确定即可添加成功。

![!快速创建场景](../../img/api/接口自动化模块/批量添加测试计划002.png){ width="900px" }

### 2.15 批量创建性能测试
!!! ms-abstract "" 
    在场景列表中选中多个场景后点击批量操作按钮中的【批量创建性能测试】，支持将多个场景导入性能测试。

![!快速创建场景](../../img/api/接口自动化模块/批量创建性能测试001.png){ width="900px" }
![!快速创建场景](../../img/api/接口自动化模块/批量创建性能测试002.png){ width="900px" }
...
![!快速创建场景](../../img/api/接口自动化模块/定时任务001.png){ width="900px" }

!!! ms-abstract "" 
    配置好时间表达式，下方会展示最近 5 次运行时间，点击【确定】按钮保存成功。

![!快速创建场景](../../img/api/接口自动化模块/定时任务003.png){ width="900px" }

!!! ms-abstract "" 
    任务通知支持配置事件、接收人、接收方式。

![!快速创建场景](../../img/api/接口自动化模块/定时任务004.png){ width="900px" }

### 2.17 场景导出导入
!!! ms-abstract "" 
    场景列表支持导出【MeterSphere 格式】和【JMETER 格式】 两种格式。
![!快速创建场景](../../img/api/接口自动化模块/场景列表导出001.png){ width="900px" }

!!! ms-abstract "" 
    场景列表支持导入【MeterSphere】、【Postman】、【JMeter】、【HAR】四种格式，场景导入支持【覆盖】和【不覆盖】两种模式。

![!快速创建场景](../../img/api/接口自动化模块/场景列表导入001.png){ width="900px" }
![!快速创建场景](../../img/api/接口自动化模块/场景列表导入002.png){ width="900px" }

!!! ms-abstract "" 
    场景在跨项目、跨工作空间甚至是跨服务迁移数据导入、导出时，能够保留自身步骤的引用关系，保证数据的完整性和一致性。
![!快速创建场景](../../img/api/接口自动化模块/场景列表导入003.png){ width="900px" }

!!! ms-abstract "" 
    同一工作空间下，不同项目迁移数据导入
![!快速创建场景](../../img/api/接口自动化模块/场景列表导入004.png){ width="900px" }
...
![!快速创建场景](../../img/api/接口自动化模块/查看引用001.png){ width="900px" }
![!快速创建场景](../../img/api/接口自动化模块/查看引用002.png){ width="900px" }

### 2.19 将场景移入回收站
!!! ms-abstract "" 
    在场景列表中点击指定场景操作列中的【删除】按钮，该场景将被移入回收站，同时场景变为【废弃】状态。用户可以点击左侧模块树中的【回收站】查看已废弃的场景。

![!快速创建场景](../../img/api/接口自动化模块/回收站000.png){ width="900px" }
![!快速创建场景](../../img/api/接口自动化模块/回收站001.png){ width="900px" }

### 2.20 恢复场景
!!! ms-abstract "" 
    点击左侧模块树中的【回收站】，进入回收站页面，选择要恢复的场景点击操作列中的【恢复】按钮。
![!快速创建场景](../../img/api/接口自动化模块/回收站002.png){ width="900px" }

### 2.21 彻底删除场景
!!! ms-abstract "" 
    点击左侧模块树中的【回收站】，进入回收站页面，选择要彻底删除的场景点击操作列中的【删除】按钮。 从回收站中删除某个场景时，该场景将从数据库中删除且无法恢复，请谨慎使用该功能。

![!快速创建场景](../../img/api/接口自动化模块/回收站004.png){ width="900px" }
