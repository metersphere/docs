...
MeterSphere提供了多种不同类型和范围的标准代码模板，同时还支持用户自定义代码模板。代码模版分为：API测试、自定义变量、项目环境、自定义代码片段、异常处理、报文处理。
![!代码模板](../../img/api/代码模板.png)

#### API测试
从API定义导入可以直接导入API或CASE自动生成脚本。
...
#### 报文处理
可自动生成修改报文头添加长度、隐藏报文头的脚本。
![!报文处理](../../img/api/报文处理.png)
...
!!! info "说明"
通过${regname`_`n}进行引用，n为行数，${regname`_`1}为regname这列的第一行值。
...
文档格式选择 `JSon` ，点击 `推荐JSONPath断言` 按钮可直接从响应体信息中选择要断言信息。
...
文档格式选择 `XML` 添加校验。
![!文档结构校验](../../img/api/XML文档结构校验.png)

勾选`跟随API定义`默认不校验。
![!文档结构校验](../../img/api/XML文档结构校验1.png)

点击`导入`可导入需断言xml文件，自动生成文档结构校验元素表格。
