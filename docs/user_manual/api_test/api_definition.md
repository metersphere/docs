## 1 概述
!!! ms-abstract "" 
	接口定义展示当前项目的所有接口数据，包含接口模块树、api接口列表、接口CASE列表、文档四个部分。
...
### 3.4 版本管理 (X-Pack)
!!! ms-abstract ""
	点击页面右上角的【版本: v1.1.0】可查看所有的版本号，选择当前版本号，创建接口。在功能或接口用例列表页面，列表默认展示所有接口的最新版本数据，并支持批量操作复制指定版本的 CASE 和 Mock 数据。可快速对接口做版本切换及历史数据对比，优化对接口的管理方法。
...
### 3.7 SQL请求
!!! ms-abstract ""
	一个项目支持配置多个数据源，选择运行环境，根据接口实际使用场景选择具体数据源，编写sql脚本，存储结果是将返回结果的所有字段存储到一个变量中。<br />
...
### 3.9 依赖关系设置
!!! ms-abstract ""
	接口定义支持设置接口的前后置依赖关系。点击【添加】按钮，可以添加前置接口依赖、后置接口。
...
### 5.1 列表功能
!!! ms-abstract ""
	支持对接口用例的 编辑 执行 复制 删除鼠标悬浮【…】可展示更多功功能、查看引用、执行历史、创建性能测试操作。<br>
	* 【编辑】：编辑该接口用例 <br>
	* 【执行】：调试该接口 <br>
	* 【复制】：复制该接口用例信息 <br>
	* 【删除】：删除该接口用例到回收站 <br>
...
!!! ms-abstract ""
	...
	* 【复制版本数据】(X-Pack)：将指定版本的 CASE 和 Mock 数据复制到当前版本中<br>
