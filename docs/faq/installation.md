...
> 配置文件说明请参考 [修改安装配置(可选)](/docs/v1.20.x-lts/installation/online_installation/#32)。

## 3 如何在 Kubernetes 中搭建 MeterSphere？

可以参照我们提供的 [helm chart](https://github.com/metersphere/helm-chart)。
详见在线文档/安装部署/Kubernetes中部署：https://metersphere.io/docs/v1.20.x-lts/installation/kubernetes_installation/
...
下载对应文件名带 86 的 flyway sql，然后重新手动一条条执行下，全部执行成功后修改 metersphere_version 表对应版本的 success 值为1，然后 msctl reload 重启服务即可。
...
## 24 怎样监控被压测的机器
在被测服务器上安装 node-exporter 服务，然后在性能测试中高级配置里添加监控，填写被测服 node-exporter 服务的 ip 和端口以及监控项
