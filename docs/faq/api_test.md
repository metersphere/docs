...
手动执行的接口自动化场景不会自动生成测试报告，用户需要点击该场景的最后执行结果手动保存测试报告。请参考[执行指定场景](/docs/v1.20.x-lts/user_manual/api_test/api_automation/#_12)
...
## 27 MeterSphere可以直接连接开发工具传输API吗？
...
## 37 MeterSphere 怎么使用 CSV文件
1.接口自动化中可以将 CSV 文件作为场景变量使用，配合循环控制器使用；
2.可以在性能测试中作为参数被引用；
